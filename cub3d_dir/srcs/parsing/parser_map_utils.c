...
int	ft_ismapclosed(char **map, int i, int j)
{
	if (ft_checksides(map) == 1)
		ft_error(ERR, "Error: Map is not closed on the side, sorrey\n");
	while (map[i])
	{
		j = 0;
		while (map[i][j])
		{
			if (map[i][j] != '1')
			{
				if (ft_checkup(map, i, j) == 1)
					ft_error(ERR, "Error: Map is not closed on the top, sorrey\n");
				else if (ft_checkdown(map, i, j) == 1)
					ft_error(ERR, "Error: Map is not closed on the bottom, sorrey\n");
			}
			j++;
		}
		i++;

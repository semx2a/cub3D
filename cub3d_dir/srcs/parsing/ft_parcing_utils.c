...
int	ft_get_path(t_game *game, char *str, char **path, int nb)
{
	...
	i = 2;
	if (str[i] != ' ')
		return (ft_error("Error: space missing before texture path\n"));
	while (str[i] == ' ')
		i++;
	*path = ft_strdup(str + i);
	if (path == NULL)
		return (ft_error("Error: Cannot allocate path for texture\n"));
	if (nb == 1)
		game->check.no++;
	if (nb == 2)
		game->check.so++;
...
int	ft_get_info(t_game *game, char *str)
{
	if (str[0] == 'N' && str[1] == 'O')
		...
		return (ft_error("Error: Invalid character in file\n"));
	return (0);
}

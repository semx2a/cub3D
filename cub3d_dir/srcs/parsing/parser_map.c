	...
	while (game->map[i])
	{
		...
		while (game->map[i][j])
		{
			if (ft_lachose(game->map[i][j]) == 0)
				ft_error(ERR, "Error: Eww, this is not a proper map, sorrey\n");
				...
				if (ft_replace_space(game->map, i, j) == 1)
					ft_error(ERR, "Error: Invalid spaces in map, sorrey\n");
		j++;
		}
		i++;
	}
	if (pos != 1)
		ft_error(ERR, "Error: Wrong number of player, sorrey\n");
	return (0);
}
...
int	ft_map_parser(t_game *game, char **tab, int begining)
{
	...
	if (game->map == NULL)
		ft_error(ERR, "Error: Map allocation has failed\n");
	game->map[i] = NULL;
	i = 0;
	while (tab[begining])
	{

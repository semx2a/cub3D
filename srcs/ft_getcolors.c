...
int ft_basic_checkcolors(char *str)
{
    ...
    while (str[i])
    {
        if ((str[i] < '0' && str[i] > '9') && str[i] != ' ' && str[i] != ',')
            return (ft_error("Error: Invalid characters in color\n"));
        i++;
    }
    return (0);
}
...
int ft_validcolor(t_game *game, int rgb[3], char *str)
{
    ...
    while(i < 3)
    {
        if (rgb[i] < 0 || rgb[i] > 255)
            return (ft_error("Error: That's not a color, sorrey\n"));
        i++;
    }
    if (str[0] == 'F')
    {
...
int ft_getcolors(t_game *game, char *str, int i)
{
    ...
    tab = ft_split(str + 1, ',');
    if (ft_tablen(tab) != 3)
        return (ft_error("Error: Colors size is way too big, sorrey\n"));
    while(tab[i])
    {
        ...
        if (ft_strlen(tronc) > 3)
        {
            ft_free_stab(tab);
            free(tronc);
            return(ft_error("Error: Invalid colors, sorrey\n"));
        }
        rgb[i] = ft_atoi(tronc);
        free(tronc);
        i++;

...
static	int	text_init(t_game *game, int i)
{
	...
	if (!(game->tex.tex = (unsigned int **)
		ft_calloc(sizeof(unsigned int *), 4)))
		return (ft_error("texture alloc memory failed"));
	if (!(game->tex.addr = (unsigned int **)
		ft_calloc(sizeof(unsigned int *), 4)))
		return (ft_error("texture alloc memory failed"));
...
int	load_texture(t_game *game)
{
	...
	while (i < 4)
	{
		if (!(game->tex.img[i] = mlx_xpm_file_to_image(game->mlix,
		game->textpath[i], &game->tex.width[i], &game->tex.height[i])))
			return (ft_error("Can't get textures, sorrey"));
		...
		game->tex.height[i], sizeof(unsigned int))))
			return (ft_error("Texture alloc memory failed, sorrey"));
		j = 0;

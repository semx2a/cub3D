...
int	ft_kurkipucheck(t_check check)
{
    ...
    printf("%d\n", check.c);*/
	if (check.no != 1)
		return(ft_error("Error: Only one texture is allowed for north, sorrey \n"));
	if (check.so != 1)
		return(ft_error("Error: Only one texture is allowed for south, sorrey \n"));
	if (check.ea != 1)
		return(ft_error("Error: Only one texture is allowed for east, sorrey \n"));
	if (check.we != 1)
		return(ft_error("Error: Only one texture is allowed for west, sorrey \n"));
	if (check.f != 1)
		return(ft_error("Error: Only one color is allowed for the floor, sorrey \n"));
	if (check.c != 1)
		return(ft_error("Error: Only one color is allowed for the ceiling, sorrey \n"));
	return (0);
}
...
int	ft_parcefile(t_game *game, char **file)
{
	...
	if (ft_mapparcer(game, file, begining) == 1)
		return (1);
...
int	ft_parcing(t_game *game, char *file)
{
	...
	(void)game;
	if (ft_iscub(file) == 1)
		return (ft_error("Error: it's not a .cub, sorrey!\n"));
	copyfile = ft_getfilecontent(file);
	if (copyfile == NULL)
		return (ft_error("Error: Cannot open or read file, sorrey \n"));
